...
select
 to_char(statement_date-1,'YYYY-MM') as report_mth
,case
...
,row_number() over (partition by b.business_id, a.statement_date order by a.record_version desc) as rn
,c.fico_score
,app_booking.created_at as booking_timestamp
from PROD_DB.DATA.CREDIT_CARD_ACCOUNT_LOAN_TAPE_HISTORY a
left join FIVETRAN_DB.PROD_NOVO_API_PUBLIC.CREDIT_CARD_ACCOUNTS b
on a.account_id = b.external_account_id
left join (select * from most_recent_invite where rn = 1) c
...
left join (
    ...
    group by 1
) app_booking on app_booking.business_id = b.business_id
where b.business_id not in (select business_id from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.BUSINESS_GROUP_ASSIGNMENTS where business_group_id = '75fe98d2-6549-46a1-aa04-a1c621e21d9e')
...
select
 to_char(a.created_at,'YYYY-MM') as booking_mth,
 case
 when b.fico_score between 581 and 619 then '580-620'
 when b.fico_score between 620 and 659 then '620-660'
 when b.fico_score between 660 and 719 then '660-720'
...
on a.business_id = ca.business_id
...
select
 business_id
,min(created_at) as booking_timestamp
from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.CREDIT_CARD_APPLICATIONS
where status = 'APPROVED'
...
,case
 when b.is_activated = 'TRUE' then 'Activated'
 else 'Unactivated'
 end as activated_flag
,count(a.business_id) as acct_vol
from cc_bookings a
left join (select * from latest_event where rn = 1) b
on a.business_id = b.business_id
left join (select * from most_recent_invite where rn = 1) c
...
select
 ...
 'Aggregate' as cc_fico_bin,
 count(case when b.status = 'APPROVED' and datediff(day,a.created_at,b.submitted_at) between 0 and 30 then a.business_id end)/count(a.business_id) as approval_rate,
 count(case when b.submitted_at is not null and datediff(day,a.created_at,b.submitted_at) between 0 and 30 then a.business_id end)/count(a.business_id) as response_rate
...
select
 ...
 count(case when b.status = 'APPROVED' and datediff(day,a.created_at,b.submitted_at) between 0 and 31 then a.business_id end)/count(a.business_id) as approval_rate,
 count(case when b.submitted_at is not null and datediff(day,a.created_at,b.submitted_at) between 0 and 31 then a.business_id end)/count(a.business_id) as response_rate
from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.CREDIT_CARD_INVITATIONS a
left join FIVETRAN_DB.PROD_NOVO_API_PUBLIC.CREDIT_CARD_APPLICATIONS b
on a.business_id = b.business_id
where a.business_id not in (select business_id from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.BUSINESS_GROUP_ASSIGNMENTS where business_group_id = '75fe98d2-6549-46a1-aa04-a1c621e21d9e')
...
,round(sum(payment_allocated_interest/100),2) as interest_collected
,round(sum(payment_allocated_fees/100),2) as fees_collected
,round(sum(payment_allocated_principal/100),2) as principal_collected
,count(distinct business_id) as open_vol
from (select * from loan_tape_updated where rn = 1)
group by 1,2
),
...
chargeoff as (
select
 to_char(dateadd(day,-1,statement_date::date),'YYYY-MM') as report_mth
,case
 when fico_score between 581 and 619 then '580-620'
 when fico_score between 620 and 659 then '620-660'
 when fico_score between 660 and 719 then '660-720'
...
select
 to_char(statement_date,'YYYY-MM') as report_mth
,round(sum(payment_allocated_principal/100),2) as principal_payment
from (select * from loan_tape_updated where rn = 1)
group by 1
order by 1
)
...
select
 ...
 'aggregate' AS CC_FICO_BIN,
 count(case when a.days_past_due between 180 and 210 then a.business_id end) as co_unit_vol,
 coalesce(sum(case when a.days_past_due between 180 and 210 then ending_balance end/100),0) as co_dollar_total,
...
select
 ...
 'aggregate' AS CC_FICO_BIN,
 count(case when a.days_past_due between 180 and 210 then a.business_id end) as co_unit_vol,
 sum(case when a.days_past_due between 180 and 210 then ending_balance/100 end) as co_dollar_total,
 count(case when a.days_past_due <= 210 then a.business_id end) as open_vol
from loan_tape_statement_join a
where a.business_id not in (select business_id from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.BUSINESS_GROUP_ASSIGNMENTS where business_group_id = '75fe98d2-6549-46a1-aa04-a1c621e21d9e')
and a.billing_period_number != 0
...
-- where business_id not in (select business_id from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.BUSINESS_GROUP_ASSIGNMENTS where business_group_id = '75fe98d2-6549-46a1-aa04-a1c621e21d9e')
...
-- where business_id in (select business_id from mca_open)
-- ),
-- most_recent_invite as (
-- select
--  *
...
-- select
--  statement_date
-- ,sum(ending_balance/100) as os_total --Total Originations
...
-- ,sum(case when days_past_due >= 60 then ending_balance/100 end) as bucket3plus_os_total
-- ,sum(case when days_past_due >= 180 then ending_balance/100 end) as co_dollar_total
-- ,count(distinct a.business_id) as open_vol
...
-- ,count(case when days_past_due >= 60 then a.business_id end) as bucket3plus_acct_total
-- ,count(case when days_past_due >= 180 then a.business_id end) as co_acct_total
...
-- on a.business_id = b.business_id
-- where billing_period_number >= 1
-- and to_char(b.created_at,'YYYY-MM') <= '2025-04'
-- and day(statement_date) = 1
-- and datediff(month,statement_date,current_date) = 0
-- and a.business_id in (select business_id from mca_cc_doubledip)
-- group by 1
-- ;

-- --mca double dip performance--
-- with mca_open as (
-- select
--  business_id
-- from fivetran_db.prod_novo_api_public.lending_businesses
...
-- where b.business_id not in (select business_id from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.BUSINESS_GROUP_ASSIGNMENTS where business_group_id = '75fe98d2-6549-46a1-aa04-a1c621e21d9e')
-- and a.billing_period_number >= 1
-- and day(a.statement_date) = 1
-- and datediff(day,b.created_at,a.statement_date) >= 46
-- ),
-- final as (
-- select
--  to_char(a.statement_date-1,'YYYY-MM') as report_mth,
--  'aggregate' AS CC_FICO_BIN,
--  round(sum(a.ending_balance/100),2) as total_outstanding,
--  count(case when a.days_past_due > 0 then a.business_id end) as dq_1plus_count,
--  count(case when a.days_past_due >= 60 then a.business_id end) as dq_60plus_count,
--  round(sum(case when a.days_past_due > 0 then a.ending_balance/100 end),2) as dq_1plus_dollar,
--  round(sum(case when a.days_past_due >= 60 then a.ending_balance/100 end),2) as dq_60plus_dollar,
--  round(sum(a.daily_balance_purchases/100),2) as total_adb,
--  round(sum(a.credit_limit/100),2) as total_credit_limit,
--  round(sum(case when a.grace_period = 'FALSE' then a.daily_balance_purchases/100 end),2) as total_stfc,
--  count(distinct a.business_id) as open_vol
-- from (select * from loan_tape_updated where rn = 1) a
-- where a.business_id not in (select business_id from FIVETRAN_DB.PROD_NOVO_API_PUBLIC.BUSINESS_GROUP_ASSIGNMENTS where business_group_id = '75fe98d2-6549-46a1-aa04-a1c621e21d9e')
-- and a.billing_period_number != 0
-- and business_id in (select distinct business_id from mca_cc_doubledip)
-- group by 1,2
-- union all
-- select
--  to_char(a.statement_date-1,'YYYY-MM') as report_mth,
...
-- and day(a.statement_date) = 1
-- and datediff(day,b.created_at,a.statement_date) >= 46
-- and b.business_id in (select business_id from prod_db.adhoc.cc_account_flags where multi_acct_flag = 1)
-- ),
-- final as (
-- select
--  to_char(a.statement_date-1,'YYYY-MM') as report_mth,
...
-- select
--  distinct business_id
-- ,sum(principal_balance_outstanding) as os
-- ,sum(case when days_past_due = 0 then principal_balance_outstanding end) as current_os
-- ,sum(case when days_past_due between 1 and 29 then principal_balance_outstanding end) as bucket1_os
-- ,sum(case when days_past_due between 30 and 59 then principal_balance_outstanding end) as bucket2_os
-- ,sum(case when days_past_due >= 60 then principal_balance_outstanding end) as bucket3plus_os
-- ,sum(case when days_past_due >= 180 then principal_balance_outstanding end) as co_os
-- from prod_db.data.loan_tape
-- where snap_date = '2025-05-01'
-- and business_id in (select business_id from mca_cc_doubledip)
-- group by 1
-- )
-- select
--  sum(os) as os
-- ,sum(current_os) as current_os
-- ,sum(bucket1_os) as bucket1_os
-- ,sum(bucket2_os) as bucket2_os
-- ,sum(bucket3plus_os)-19074.27 as bucket3plus_os
-- ,sum(co_os) as co_os
-- ,count(business_id) as open_vol
-- ,count(case when current_os >= 0 then business_id end) as current_vol
-- ,count(case when bucket1_os > 0 then business_id end) as bucket1_vol
-- ,count(case when bucket2_os > 0 then business_id end) as bucket2_vol
-- ,count(case when bucket3plus_os > 0 then business_id end)-1 as bucket3plus_vol
-- ,count(case when co_os > 0 then business_id end) as co_vol
-- from mca_performance
-- ;